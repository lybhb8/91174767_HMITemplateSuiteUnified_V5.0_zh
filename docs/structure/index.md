...
<html><body><table><tr><td></td><td>HMI Template Suite (WinCCUnified)_V19</td></tr><tr><td>1</td><td>Types</td></tr><tr><td>2</td><td>Master copies</td></tr><tr><td>3</td><td>Commondata</td></tr><tr><td>4</td><td>Languages&resources</td></tr></table></body></html>  

> 注：有关将该库集成到您项目中的信息，请参阅第5.1节(#)。 
...
> 注：有关将库集成到TIA Portal项目的说明，请参见第5.1节。 
...
画面名称及其定义的相关信息详见第4.2节。  

所有画面的主副本在不同分辨率下均采用相同结构，包含4个不同子文件夹（如图2-5所示），具体说明详见后续子章节2.1.3.1至2.1.3.4。 
...
>注:布局和导航概念的详细说明可参见第4.1节。 

#### 2.1.3.2. 02 – Screens(画面)  

在“02 – Screens”文件夹中，您将找到预配置的HMI屏幕，例如仪表板、概览屏幕或向导（参见第4.3节）。此外，单个屏幕对象（如按钮、文本框等）作为模板存在于“0000_CopyTemplate_single_objects”屏幕中（参见第4.4节）。

![](../images/6b5a7585ae22725320860c6a779fafd828dfda30af8bae3e80dc183d40e8a80a.jpg)  

#### 2.1.3.3. 03 – Popup Screens (弹出画面) 

弹出窗口用于显示机器警报或更改参数，通常用于输入数值。常规屏幕（非弹出窗口，例如来自“02-Screens”库文件夹的屏幕）仅应显示输出值，而弹出窗口则用于编辑数值。因此弹出窗口主要用于向用户清晰区分输出与输入区域。但库文件及预配置设备中存在部分例外示例界面。
...
> 注：预配置的弹出窗口是指在WinCC Unified中以弹出窗口形式调用的界面。有关如何在界面中访问弹出窗口的信息，请参阅第5.4节。
...
文件夹"04 – Other Templates"包含3个子文件夹： "4100_Option Panel" (Section 4.3.5), "4200_Notifications"(Section 4.3.6) 和“"4300_Function Panel" (Section 4.3.7)。

### 2.1.4. WinCC Unified View Of Things  

该模板专为SIMATIC WinCC Unified “View of Things”（简称“VoT”）设计。  

SIMATIC WinCC Unified “View of Things”是一款存储于SIMATIC S7-1500的Web应用程序。有关“VoT”的详细信息可通过以下链接查阅\5\。  
...
> 注: 为符合VoT限制，请勿配置超过10个屏幕。文件夹 "00 - Info: Use up to ten screens for your VoT application!" 内不含任何屏幕，仅供参考。
...
有关各个面板及其配置的更详细说明，请参见Section 5.5。 
